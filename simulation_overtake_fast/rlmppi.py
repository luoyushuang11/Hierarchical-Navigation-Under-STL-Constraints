...
# ==========================================
# 2. Dynamic window logic (adapted to virtual clock and dynamic obstacles)
# ==========================================
@jax.jit
def window_based_update_dynamic(aug_state, next_phys_state, subgoals_array, obstacles_params, windows_array, time_vars):
    """
    Minimalist logic update
    Args:
        aug_state: The current expansion status
        next_phys_state: The physical state of the next moment
        subgoals_array: Target parameters [offset_x, abs_y, r, t_idx, ref_obs_idx]
        obstacles_params: Dynamic Obstacle Parameter (N_obs, 4) -> [start_x, start_y, vx, radius]
        windows_array: Time window configuration
    """
    # Unpacking status
    tau = aug_state[4]
    ...
    # 1. Virtual clock advance
    tau_next = tau + 1.0
    
    # [Critical] Calculate the physical time corresponding to the logic step
    # Logical updates are performed in RL steps (0.2s per step)
...
def run_final_merged_ref_virtual():
    ...
    model_path = os.path.join(current_dir, "RLtrain/dover_model_fast_s_2.msgpack")
    ...
    print(f"✅ Preheating is complete。")
    ...
    for step in range(TOTAL_STEPS):
        ...
        if should_inc_tau:
            next_logic = window_based_update_dynamic(
                ...
                next_phys, 
                controller.subgoals, 
                controller.obs_params, # Obstacles with speed
                LOGIC_WINDOWS,
                time_vars
            )
    ...
    # 1. Static trajectory diagram (incoming obstacle parameters with velocity)
    ...
    """
    # 3. Generate MPPI Animations (MP4)
