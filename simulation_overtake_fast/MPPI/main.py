...
def main():
    ...
    # --- Run the simulation ---
    # Note: Make sure that the number of unpacks returned by simulation.py is consistent with the return of run_simulation
    ...
    # === [Optimized] Save data to .npz file ===
    import time
    ...
    print("⏳ When converting the MPPI sampled controls...")
    mppi_array = np.asarray(mppi_sampled_us)
    
    print(f"💾 When saving data to disk (using uncompressed fast save): {npz_filename} ...")
    # [Core Modification] Replace it with uncompressed np.savez, which is dozens of times faster, effectively avoiding CPU compression deadlocks
    np.savez(
        npz_filename,
    ...
    # Save static trajectory maps (including lane and obstacle initial locations)
    plot_simulation_result(states, save_path=path_trajectory_plot)

    print("\nSaving CBF variation plot...")
    # Save the CBF numerical curve
    plot_cbf_values(h_values, save_path=path_cbf_plot)
    
    # [Modification point 3]: Generate MP4 animation (uncomment)
    # Note: Make sure that the animate_simulation in your visualization.py already supports 'ffmpeg' writer
    print("\nGenerating and saving animation (MP4)...")
    #animate_simulation(states, sampled_us=mppi_sampled_us, optimal_us=global_Us, save_path=path_animation)
    print(f"Animation saved: {path_animation}")
