...
def calculate_gae(
    ...
    # Use jax.lax.scan to calculate GAE recursively in reverse order
    ...
    # Transpose to (Time, Batch) for easy scanning
    deltas_T = deltas.T
    dones_T = dones.T
...
def compute_log_prob(mean, log_std, action):
    ...
    """
    # [Critical Fix] Limit the log_std range to prevent values from exploding or collapsing
...
def critic_loss_fn(critic_params, critic_apply_fn, batch, target):
    ...
    # [Critical Fix] Force compressed dimensions
    values = jnp.squeeze(values)
...
def update_step(
    ...
    # [Critical Fix] Advantage Normalization
    # Must be done inside the update function, this is the core of PPO convergence
