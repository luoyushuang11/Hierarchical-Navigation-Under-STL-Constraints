...
def update_augmented_state(aug_state, next_phys_state, time_vars, subgoals_array, obstacles_params):
    ...
    Args:
        obstacles_params: (N_obs, 4) -> [start_x, start_y, v_x, radius]
        subgoals_array: (N_goal, 5) -> [offset_x, abs_y, r, t_idx, ref_obs_idx]
    """
    ...
    tau_next = tau + 1.0
    current_time = tau_next * TGPOConfig.DT # Current Physical Time(s)
    
    # 2. Calculate the current position of the dynamic obstacle
    ...
    # 3. Resolve the current sub-target (relative coordinates -> absolute coordinates)
    ...
    # Target x = Current x + offset of the obstacle being followed
    # Use safe ref_idx to prevent crossing the line (although usually not)
    target_obs_x = obs_x_curr[ref_obs_idx]
    goal_x_abs = target_obs_x + offset_x
    
    goal_pos_abs = jnp.array([goal_x_abs, goal_y])
    
    # Get precise target time points
    t_target = jnp.round(time_vars[t_idx])
    
    # 4. Check the atomic proposition mu (if it is in the region)
    dist_goal = jnp.linalg.norm(next_phys_state[:2] - goal_pos_abs)
    mu_satisfied = dist_goal <= goal_radius
    
    # 5. Renew certificates r （Strict Time Match）
    cond_reset = (r == 2)
    # tau_next It must be exactly equal to the target point in time sampled
    time_match = (tau_next == t_target)
    
    # Completion Conditions: (Not Finished Yet) & (Time Up) & (Space Up)
    ...
    # 7. Update dynamic security constraints chi
    # Calculate the distance to all moving obstacles
    ...
    # (B) Boundary check
    # Question requirements: 0.6 <= y <= 2.4. But because there is a target in y=3.0, 
    # To ensure solvability, it is relaxed to 3.4 (including the second lane)
    y_curr = pos[1]
