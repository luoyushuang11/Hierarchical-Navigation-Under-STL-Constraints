...
# ==========================================
# 1. Dynamic scene visualization tools
# ==========================================
def plot_dynamic_overtaking(phys_trajs, scores, scenario, iter_idx, filename="dynamic_result.png"):
    """
    Draw a trajectory map of a dynamic overtaking scene
    Display content: road boundary, dynamic obstacle trajectory, snapshot at critical moments, agent trajectory
    """
    ...
    # Create a long strip canvas (for long distances of X_LIMIT=60)
    ...
    for i in range(len(obs_params)):
        ...
        # 1. Draw the time track of the obstacle (from start to finish)
        end_x = ox + vx * (T * dt)
        ...
        # 2. Draw the location of obstacles at key points in time (snapshot)
        snapshot_steps = [0, 25, 50, 100, 150]
...
class TGPOTrainer:
    ...
    def save_checkpoint(self, filename="dover_model_slow_s_2.msgpack"):
        ckpt = {'actor': self.actor_state, 'critic': self.critic_state}
        with open(filename, 'wb') as f:
            f.write(serialization.to_bytes(ckpt))
        print(f"[Checkpoint] ✅ Model has been saved to: {filename}")

    def load_checkpoint(self, filename="dover_model_slow_s_1.msgpack"):
        if not os.path.exists(filename):
            print(f"[Checkpoint] ⚠️ Checkpoint {filename} not found, starting new training.")
            return
    ...
    def save_logs(self, filename="dynamic_training_logs.npz"):
        np.savez(filename, **self.log_history)
        print(f"[Log] ✅ Dynamic scenario training logs have been saved to: {filename}")
    ...
    def plot_training_curves(self, filename="training_curves.png"):
        ...
        plt.savefig(filename)
        plt.close()
        print(f"[Visual] The training curve diagram has been saved to: {filename}")

    
    def train(self, num_iterations, log_filename="dynamic_logs.npz"): 
...
if __name__ == "__main__":
    ...
    trainer.train(num_iterations=100, log_filename="slow_s_2.npz")