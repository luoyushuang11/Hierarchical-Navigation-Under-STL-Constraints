...
try:
    ...
    print("✅ RLtrain The module loads successfully。")
except ImportError as e:
    print(f"❌ Failed to load RLtrain: {e}")
    sys.exit(1)
finally:
...
try:
    ...
    print("✅ MPPI The module loads successfully。")
except ImportError as e:
    print(f"❌ Failed to load MPPI: {e}")
    sys.exit(1)
...
# ==========================================
# 2. Logical Updates (Integrated SFP/SDF Security Verdict)
# ==========================================
@jax.jit
def window_based_update(aug_state, next_phys_state, subgoals_array, obstacles_array, windows_array, time_vars):
    ...
    # 3. Get the Config window
    win_start = windows_array[t_idx, 0]
    win_end   = windows_array[t_idx, 1]
    time_var = time_vars[t_idx]
    
    is_last_goal = (p == (max_p - 1))
    #effective_radius = jnp.where(is_last_goal, 0.15, 0.18)
    effective_radius = jnp.where(is_last_goal, 0.3, 0.6)
    # 4. Judgment conditions
    # A. Space arrives
    dist = jnp.linalg.norm(next_phys_state[:2] - goal_pos)
    is_spatial_reached = dist <= effective_radius
    
    # B. Time is in the window
...
class HierarchicalController:
    def __init__(self, model_path):
        self.trainer = TGPOTrainer(scenario_name="local_minima_trap")
    ...
    def _compile_functions(self):
        # [Modification] Let predict_loop return both the action a and the trajectory for ZOH initialization
        def predict_loop(carry, _):
            ...
            # Return: (carry), (output track, output action)
    ...
    # [Modified] Get tracks and actions
...
# [Core modification] supports noise_scale and weight_scale for Focus Warm-up
@jax.jit
def mppi_optimize_step(current_state, U_prev, rng_key, ref_traj, current_time, time_vars, real_reach_flags, noise_scale=1.0, weight_scale=1.0):
    ...
    # Dynamically adjust tracking weights
    ...
    def rollout_scan(carry, inputs):
        ...
        # [Modified] Applies dynamic noise scale
        ...
        # [Modified] Tracking error is calculated using dynamic weights
        c_track = 1.0 * jnp.dot(next_state - ref_state, jnp.dot(Q_TRACK_SCALED, next_state - ref_state))
        
        # [Modification 2] Pass in time_vars and sim_flags, and receive the updated next_sim_flags
        h_val, next_sim_flags = h_x_jax(next_state, t_next, time_vars, sim_flags)
        c_cbf = 1e4 * jnp.clip(-h_val, 0.0, jnp.inf)
        # [Modification 3] Pass the next_sim_flags package to the next prediction
    ...
    def single_rollout_full(subkey):
        ...
        def scan_fn_u(carry, inputs):
            # [Modification 7] Unpacking sim_flags
            cost_acc, state, t_now, sim_flags = carry
            u_nominal, noise, ref_state = inputs
            
            # [Modified] Same applies noise ratio
            u = u_nominal + noise * noise_scale
            ...
            c_ctrl = jnp.dot(u, jnp.dot(R_MPPI, u))
            c_track = 1.0 * jnp.dot(next_state - ref_state, jnp.dot(Q_TRACK_SCALED, next_state - ref_state))
            # [Modification 8] Updated h_val and flag calculations
            h_val, next_sim_flags = h_x_jax(next_state, t_next, time_vars, sim_flags)
            c_cbf = 1e4 * jnp.clip(-h_val, 0.0, jnp.inf)
            # [Modification 9] Repack
            return (cost_acc + c_ctrl + c_track + c_cbf, next_state, t_next, next_sim_flags), (next_state, u)

        # [Modification 10] Discard the final simulation flag
...
def run_final_merged_ref_virtual():
    ...
    print(f"\n{'='*60}")
    print(f"🚀 Simulation of two-layer hierarchical control (Focus Warmup + SFP Safety)")
    print(f"{'='*60}")

    model_path = os.path.join(current_dir, "RLtrain/local_minima_trap_model_2balls.msgpack")
    ...
    # Initial state
    start_x = jax.random.uniform(k_x, minval=9.0, maxval=11.5)
    start_y = jax.random.uniform(k_y, minval=0.5, maxval=1.5)
    start_theta = jax.random.uniform(k_theta, minval=-np.pi, maxval=np.pi)
    ...
    # === [NEW] Initialize 3 real-world fuse markers ===
    curr_real_flags = jnp.array([False, False])
    
   # === [Step 1] Initialize MPPI (ZOH) with RL action sequences ===
    print("\n⚡ [Init] ZOH: Initializing the U_mppi with RL action sequences...")
    ...
    fine_ref = interpolate_to_mppi(curr_phys, current_coarse_ref)  # <--- Fill in this line
    ...
    # Zoom in [-1, 1] back to real physical units
    coarse_actions_physical = coarse_actions * action_scale
    
    # 4. Extended Action Sequences: (30, 2) -> (300, 2)
    k_ratio = int(DT_RL / DT_MPPI) # 10
    
    # Repeat with the zoomed-in action
    ...
    for step in range(TOTAL_STEPS):
        ...
        # --- B. MPPI Control (50Hz) ---
        # Normal operation: Use the default noise_scale=1.0, weight_scale=1.0
        ...
        # ====================================================
        # === [NEW] Absolute Braking Mechanism After Mission Completion (Hard Override) ===
        # ====================================================
        ...
        if should_inc_tau:
            ...
            if next_p != curr_p:
                ...
                print(f"   -> The clock is realigned. New Offset: {tau_offset:.2f}")
            
            if next_p >= 2 and not task_completed_flag:
        ...
        # [Modify] Extract h_x_jax return the first value of the tuple for logging
    ...
    data_save_path = os.path.join(save_dir, "ood3_data10.npz")
    np.savez_compressed(
        data_save_path,
    ...
    anim_path = os.path.join(save_dir, "v3_maze.mp4")
