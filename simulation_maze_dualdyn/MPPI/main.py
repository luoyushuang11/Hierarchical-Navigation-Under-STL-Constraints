...
def main():
    ...
    try:
        ...
        if 'Gpu' not in devices_str and 'Cuda' not in devices_str:
            ...
            print("成功： JAX 已检测到 GPU （CudaDevice）。")
    ...
    if len(computation_times) > 1:
        ...
        print("-------------------------")
        print(f"Target control frequency (config.dt): {target_freq:.2f} Hz (Namely {target_dt:.4f} seconds)")
        
        if mean_time > target_dt:
            print(f"Warning: Average calculation time ({mean_time:.4f}s) > Target step length ({target_dt:.4f}s)")
            print("  ==> This means that your controller is [not] real-time。")
        else:
    ...
    print("⏳ Transitioning MPPI sampled matrix (executing aggressive truncation and瘦身)...")
