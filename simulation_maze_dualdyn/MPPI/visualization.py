...
# 8 static obstacles: [cx, cy, half_w, half_h]
STATIC_OBSTACLES = [
...
def plot_cbf_values(h_values, save_path):
    ...
    if len(h_values) > max_steps:
        print(f"    [Tip] Data length {len(h_values)} Beyond {max_time}s, truncated display。")
    ...
    if np.any(h_values < 0):
        print("    [Warning]: CBF value is less than 0 at some point (already marked in red in the graph)!")
...
def plot_simulation_result(states, save_path):
    ...
    """
    # 3. Draw the dynamic obstacle paths
    ...
    # 5. Draw the trajectory of the main vehicle
...
def animate_simulation(states, sampled_us=[], optimal_us=None, save_path="simulation.mp4"):
    print(f"  ... Rendering animations to: {save_path}")

    max_steps = int(40.0 / DT)
    if len(states) > max_steps:
        print(f"    [Tip] Animation captures the first 40s")
        states = states[:max_steps]
    ...
    def update(frame, states):
        ...
        # --- 1. Set the view range ---
        ax.set_xlim(-1, 14)
        ax.set_ylim(-1, 12)
        ax.set_aspect('equal', adjustable='box') 
        
        # --- 2. Drawing a workspace ---
        ws_width = WS_RANGE[1] - WS_RANGE[0]
        ws_height = WS_RANGE[3] - WS_RANGE[2]
        ws_rect = Rectangle((WS_RANGE[0], WS_RANGE[2]), ws_width, ws_height,
        ...
        # --- 3. Draw static obstacles (gray blocks) ---
        ...
        # --- 4. Drawing Dynamic Obstacles (Triangle Logic) ---
    ...
    try:
        ...
        try:
            ...
            print(f"  [Downgrade] GIF saved: {save_path.replace('.mp4', '.gif')}")
...
def animate_simulation(states, sampled_us=[], optimal_us=None, save_path="simulation.mp4"):
    ...
    def update(frame, states):
        ...
        # --- 4. Draw Dynamic Obstacles (Noted) ---
        # =========================================================
        # [Modification point] In order to hide the dynamic obstacles, the following code has been commented
        # =========================================================
        # for i, phase in enumerate(DYN_OBS_PARAMS['phases']):
        #     dist_travel = DYN_OBS_PARAMS['speed'] * current_time + phase
        #     cycle = DYN_OBS_PARAMS['cycle']
        ...
        # --- 5. Draw Target (Always Displayed) ---
        for i, goal in enumerate(TARGETS):
            c = Circle(goal['pos'], goal['radius'], color='purple', alpha=0.3)
            ax.add_artist(c)
