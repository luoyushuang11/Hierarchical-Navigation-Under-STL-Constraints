...
def debug_plot(phys_trajs, scores, scenario, iter_idx, filename="final_best_traj.png"):
    ...
    if scenario.name == "local_minima_trap":
        # [Modify] Corresponding to the initialization range in reset_states
        # x: [9.0, 11.5], y: [0.5, 1.5]
        #以此画矩形: xy=(9.0, 0.5), width=2.5, height=1.0
    ...
    # [Modify] Adjust the view area and leave it slightly blank
    plt.xlim(-1.0, ws_w + 1.0)
    plt.ylim(-1.0, ws_h + 1.0)
    plt.grid(True)
    
    # Deduplication of legends
...
class TGPOTrainer:
    ...
    def save_checkpoint(self, filename="local_minima_trap_model_1balls.msgpack"):
        ckpt = {'actor': self.actor_state, 'critic': self.critic_state}
        with open(filename, 'wb') as f:
            f.write(serialization.to_bytes(ckpt))
        print(f"[Checkpoint] ✅ 模型已保存至: {filename}")

    def load_checkpoint(self, filename="local_minima_trap_model.msgpack"):
    ...
    def train(self, num_iterations): 
        ...
        # --- [Key Modification] State Reset Function ---
        def reset_states(key, batch_size):
            k1, k2, k3 = jax.random.split(key, 3)
            
            if self.scenario.name == "local_minima_trap":
                # === [Modified] Trap Scene Specific Initialization ===
                # Your Requirements:
                # x: [9.0, 11.5]
                # y: [0.5, 1.5]
                # theta: [-pi, pi]
...
if __name__ == "__main__":
    # [Critical] Switch to trap scene for training
