...
class STLEvaluator:
    def __init__(self, obstacles, subgoals):
        ...
        # ==========================================
        # 1. [Core Optimization] Pre-calculates the full trajectory of dynamic obstacles
        # ==========================================
        # Target shape: (T, N_dyn, 2)
        
        # A. Generate a physical time series (T,) -> (T, 1) for broadcasting
        times = (jnp.arange(self.T) * self.dt)[:, None] 
        
        # B. Extract parameters (1, N_dyn) for broadcasting
        # DYN_OBS_PARAMS: (N, 6)
        ...
        # C. Vectorized Computation of Reciprocating Motion (Ping-Pong Logic)
        length = y_max - y_min
        cycle = 2.0 * length
        
        # [Critical] plus offset enables inverted motion
        dist = times * speed + offset
        mod = dist % cycle
        
        # Reciprocating logic: outbound vs back
        y_traj = jnp.where(
            mod <= length,
            y_min + mod,
            y_max - (mod - length)
        ...
        # x_traj fixed unchanged, extending to (T, N)
        x_traj = jnp.tile(x_fixed, (self.T, 1))
        
        # Stack to get the final trajectory matrix: (T, N, 2)
        # self.dyn_obs_traj[t, i] is the coordinates of the ith obstacle at time t
        self.dyn_obs_traj = jnp.stack([x_traj, y_traj], axis=-1)
        
        # Pre-stored obstacle radius: (1, N) -> The radius suggestion here is consistent with the final difficulty in logic
        ...
        # --- A. Dynamic Obstacle Safety (Based on Pre-Calculated Trajectories) ---
        ...
        def workspace_func(states):
            ...
            # Combined x_min, y_min, x_max, y_max four distances
        ...
        for i in range(self.num_goals):
            ...
            # Closure binding target parameters
    ...
    def evaluate_single(self, trajectory):
        """
        Evaluating a single trajectory (T, 4)
        """
        # 1. Static security
        ...
        # 2. Dynamic safety (direct use of pre-calculated trajectories, extremely fast)
