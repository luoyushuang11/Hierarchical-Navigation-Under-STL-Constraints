...
def debug_plot(phys_trajs, scores, scenario, iter_idx, filename="final_best_traj.png"):
    ...
    if scenario.obstacles is not None:
        ...
        if obs_data.shape[1] == 4: # Rectangle [cx, cy, w, h]
            for obs in obs_data:
                cx, cy, w, h = obs
                lx = cx - w / 2.0
                ly = cy - h / 2.0
...
class TGPOTrainer:
    def __init__(self, seed=0, scenario_name="reach_avoid_simple"): 
        ...
        # 1. Environment and scene settings
    ...
    def save_checkpoint(self, filename="local_minima_trap_model_1balls_s_3.msgpack"):
        ckpt = {'actor': self.actor_state, 'critic': self.critic_state}
        with open(filename, 'wb') as f:
            f.write(serialization.to_bytes(ckpt))
        print(f"[Checkpoint] ✅ The model has been saved to: {filename}")

    def load_checkpoint(self, filename="local_minima_trap_model_1balls_s_2.msgpack"):
        if not os.path.exists(filename):
            print(f"[Checkpoint] ⚠️ No checkpoint found {filename}，A new training session will begin。")
            return
        ...
        print(f"[Checkpoint] ✅ The model has been loaded successfully! Continuing training...")

   
    def save_logs(self, filename="trap_training_logs.npz"):

        np.savez(filename, **self.log_history)
        print(f"[Log] ✅ The maze scenario training logs have been saved to: {filename}")
    ...
    def train(self, num_iterations, log_filename="trap_logs.npz"): 
        ...
        def reset_states(key, batch_size):
            ...
            if self.scenario.name == "local_minima_trap":
                ...
                v = jnp.zeros((batch_size, 1)) # Start stationary
...
if __name__ == "__main__":
    ...
    trainer.train(num_iterations=100, log_filename="trap_v2_s_2.npz")