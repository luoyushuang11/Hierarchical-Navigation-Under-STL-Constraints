...
def main():
    ...
    # (Keep your random theta initialization)
    ...
    # ... (The Performance Analysis report section remains unchanged) ...
    print("\n--- Performance analysis (control loops.)) ---")
    if len(computation_times) > 1:
        ...
        print(f"JIT compile time (first call.)): {computation_times[0]:.4f} seconds")
        print(f"Stable calculation time (average): {mean_time:.4f} seconds")
        print(f"Stable calculation time (fastest): {min_time:.4f} seconds")
        print(f"Stable calculation time (slowest): {max_time:.4f} seconds")
        print(f"Average Control Frequency (Stable): {mean_freq:.2f} Hz")
        
        target_dt = config.dt
        target_freq = 1.0 / target_dt
        print("-------------------------")
        print(f"Target control frequency (config.dt): {target_freq:.2f} Hz ( {target_dt:.4f} seconds)")
        ...
        if mean_time > target_dt:
            ...
            print(f"Success: Average calculation time ({mean_time:.4f}s) <= Target step length ({target_dt:.4f}s)")
            print("  ==> This means that your controller is [real-time]。")
            
    else:
        print("The simulation runtime is too short to analyze performance。")
    print("-------------------------")
    import time
    ...
    print("⏳ Transitioning MPPI sampling matrix (executing aggressive truncation)...")
    ...
    # --- 3. Visualization results (modified to save) ---

    print("\nCBF change plots are being saved...")
    #plot_cbf_values(h_values, save_path=path_cbf_plot)

    
    print("\nFinal trajectory plots are being saved...")
    #plot_simulation_result(states, save_path=path_trajectory_plot)

    print("\nCBF change plots are being saved...")
    #plot_cbf_values(h_values, save_path=path_cbf_plot)
    
    # --- 4. Optional: Generate animation (modified to save, but keep comments) ---
    # print("\nSaving animation (which can be time-consuming)...")
