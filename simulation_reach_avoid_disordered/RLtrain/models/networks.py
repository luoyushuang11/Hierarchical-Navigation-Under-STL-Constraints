...
class Actor(nn.Module):
    ...
    def __call__(self, aug_state, time_vars):
        ...
        # Logical state (p_prev, p, r, chi) - Small value, directly entered
        logic_state = aug_state[..., TGPOConfig.IDX_P_PREV:]
        
        #  TIME_SCALE -> [0, 1]
        # (Batch, N_vars) -> (Batch, N_vars)
        ...
        # Mean: Use Tanh to limit to [-1, 1], which corresponds to ACTION_SCALE in dynamics
...
class Critic(nn.Module):
    ...
    def __call__(self, aug_state, time_vars):
        ...
        """
        # 1. Normalization (consistent with Actor)
        phys_state = aug_state[..., TGPOConfig.IDX_PHYS]
        norm_phys = phys_state / TGPOConfig.PHYS_SCALES
