...
# ==========================================
# 1. Trajectory Visualizer (Remains the same)
# ==========================================
...
class TGPOTrainer:
    ...
    def save_checkpoint(self, filename="tgpo_model_3goals_s_7.msgpack"):
    ...
    def load_checkpoint(self, filename="tgpo_model_3goals_s_6.msgpack"):
        if not os.path.exists(filename):
            print(f"[Checkpoint] ⚠️ Checkpoint {filename} is not found and a new training will start。")
            return
    ...
    def save_logs(self, filename="training_logs.npz"):
       
        np.savez(filename, **self.log_history)
        print(f"[Log] ✅ The training curve data is saved in its entirety: {filename}")
    ...
    def train(self, num_iterations, log_filename="training_logs.npz"): 
...
if __name__ == "__main__":
    ...
    trainer.train(num_iterations=100, log_filename="v1_stage_7.npz")