...
def jax_dynamics(state, control, dt=default_dt):
    ...
    # 2. Calculate State Derivative
    # [dx, dy, dtheta, dv]
    # This writing allows JAX to recognize it as a single vector operation
    state_dot = jnp.array([
        v * jnp.cos(theta),  # dx
        v * jnp.sin(theta),  # dy
        omega,               # dtheta 
    ...
    # Limit only speed (v), which is the most critical safety constraint
...
# Numpy version remains the same for debug
