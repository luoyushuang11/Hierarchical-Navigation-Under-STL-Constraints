...
def h_x_jax(x, t, time_vars, reach_flags):
    ...
    # Status update: If the history is already true, or if this frame happens to be triggered, then update to true
    ...
    # Suppose the end of target 3 is 60s
    b13_hard = jnp.where(t > 60.00, jnp.inf,
                jnp.where(flag3_new, jnp.inf, b13_hard_val))

    # Indicates obstacles
    b2 = - 2.89 + jnp.power(x1 + 2.5, 2) + jnp.power(x2, 2)
    b3 = - 1.69 + jnp.power(x1, 2) + jnp.power(x2 + 5.0, 2)
    b4 = - 1.69 + jnp.power(x1 - 2.5, 2) + jnp.power(x2, 2)
    b5 = - 1.44 + jnp.power(x1, 2) + jnp.power(x2 - 5.0, 2)
    b6 = - 1.44 + jnp.power(x1 + 2.5, 2) + jnp.power(x2 + 3.0, 2)
    b7 = - 1.44 + jnp.power(x1 + 1.0, 2) + jnp.power(x2 - 2.5, 2)
    b8 = - 1.44 + jnp.power(x1 - 5.0, 2) + jnp.power(x2 - 5.0, 2)
    b9 = - 1.69 + jnp.power(x1 - 6.0, 2) + jnp.power(x2, 2)
    b10 = - 1.44 + jnp.power(x1 + 5.0, 2) + jnp.power(x2 - 2.5, 2)
