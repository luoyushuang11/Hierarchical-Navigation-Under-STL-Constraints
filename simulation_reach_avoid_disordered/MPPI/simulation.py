...
def run_simulation(initial_state, initial_U, rng_key):
    ...
    print(f"Start the simulation... (Total duration: {config. T}s, step: {config.dt}s)")
    ...
    for t in np.arange(0, config.T, config.dt):
        ...
        if (t * 10) % (config.T) == 0:
             print(f"  Simulate progress: {t:.1f}s / {config.T}s")

    print("The simulation is complete。")
    
    # --- 5. Return timing results ---
    return (
