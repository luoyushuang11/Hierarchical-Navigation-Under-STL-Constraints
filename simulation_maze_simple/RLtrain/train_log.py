...
class TGPOTrainer:
    ...
    def save_logs(self, filename="trap_training_logs.npz"):
        """将 log_history All curve data in the file is saved to a local file"""
        np.savez(filename, **self.log_history)
        print(f"[Log] ✅ Maze scenario training logs have been saved to: {filename}")
    ...
    def plot_training_curves(self, filename="training_curves.png"):
        ...
        plt.savefig(filename)
        plt.close()
        print(f"[Visual] Training curve diagram has been saved to: {filename}")
