...
def run_final_merged_ref_virtual():
    ...
    print(f"\n{'='*60}")
    print(f"🚀 Double-Layer Hierarchical Control Simulation (Focus Warmup + SFP Safety)")
    print(f"{'='*60}")

    model_path = os.path.join(current_dir, "RLtrain/local_minima_trap_model_noballs.msgpack")
    ...
    print("\n⚡ [Init] ZOH: Utilizing RL action sequence initialization U_mppi...")
    ...
    print(f"✅ Warm-up completed. Control sequence is aligned.")
    
    print(f"\n⚡ Starting simulation (40s)...")
    ...
    for step in range(TOTAL_STEPS):
        ...
        if should_inc_tau:
            ...
            if next_p != curr_p:
                ...
                print(f"   -> The clock is realigned。 Offset: {tau_offset:.2f}")
            
            if next_p >= 2 and not task_completed_flag:
                print(f"🎉 The task is T={t_now:.2f}s Done!")
