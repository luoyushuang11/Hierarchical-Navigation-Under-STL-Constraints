...
# [x_fixed, y_min, y_max, speed, radius, offset]
...
def plot_cbf_safety(time_axis, cbf_values, save_path):
    ...
    print(f"📊 CBF safety plot is being drawn: {save_path} ...")
...
def generate_mppi_animation(
    ...
    print(f"   -> Set video frame rate FPS={real_time_fps} (to match real time)")
    ...
    try:
        ...
        print(f"   Reason: {e}")
        print("   Hint: Please check if FFmpeg is installed. If it fails, try saving as GIF.")
...
def generate_mppi_animation(
    ...
    try:
        ani.save(save_path, writer='ffmpeg', fps=real_time_fps, dpi=200, bitrate=3000)
        print(f"\n✅ Animation generation completed: {save_path}")
    except Exception as e:
        print(f"\n❌ Error: Failed to generate MP4.")
        print(f"   Reason: {e}")
