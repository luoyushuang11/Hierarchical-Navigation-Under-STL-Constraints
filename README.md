...
* **Low-Frequency High-Level Planner:** Acting as the high-level planner, the algorithm samples the optimal arrival time points for various sub-target regions at the initial stage of the task. It simultaneously generates a reference trajectory in real time to guide the low-level system.
* **Dual Time-Varying Constraint Satisfaction for STL:** To strictly satisfy the temporal logic requirements defined by STL, the low-level controller constructs two distinct Time-Varying constraints:
